...
function App() {
    ...
    if (auth !== 'authenticated') return <div className="auth-shell"><main className="auth-layout"><section className="auth-brief" aria-label="ForgeUnits Control Panel"><figure className="auth-logo"><img src="/forgeunits-logo-transparent.png" alt="ForgeUnits" /><figcaption>Control Panel</figcaption></figure><p className="auth-brief-copy">Supervisa el consumo de IA, revisa límites y mantiene cada decisión de uso trazable desde un solo centro operativo.</p><ul className="auth-signals" aria-label="Capacidades del panel"><li>Consumo y costos en tiempo real</li><li>Claves y proyectos protegidos</li><li>Alertas operativas verificables</li></ul></section><form className="auth-card" onSubmit={login} aria-labelledby="loginTitle"><div className="auth-card-head"><div className="auth-brand"><span className="auth-brand-mark"><img src="/forgeunits-logo-transparent.png" alt="" /></span><span><strong>ForgeUnits</strong><small>Workspace access</small></span></div><span className="secure-mark" aria-label="Acceso protegido">◉</span></div><div className="auth-header"><p className="eyebrow">QUOTAMANAGER</p><h1 id="loginTitle">Iniciar sesión</h1><p className="muted">Accede al panel de control de tu workspace.</p></div>{auth === 'checking' ? <p className="muted auth-loading">Comprobando acceso…</p> : <><label>Contraseña<input id="password" name="password" type="password" value={password} onChange={event => setPassword(event.target.value)} autoFocus required autoComplete="current-password" /></label><button className="primary-button" id="loginBtn" type="submit">Entrar al dashboard</button>{loginError && <p className="auth-error" role="alert">{loginError}</p>}</>}</form></main></div>
    ...
    return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark brand-logo"><img src="/forgeunits-logo-transparent.png" alt="ForgeUnits" /></span><div><strong>ForgeUnits <span>Quotamanager</span></strong><small>Control Panel · AI usage</small></div></div><div className="workspace-label">WORKSPACE</div><div className="workspace"><span className="workspace-icon">F</span><span>ForgeUnits · IA</span></div><nav>{navItems.map((item, index) => <button key={item} className={`nav-item ${activeNav === item ? 'active' : ''}`} onClick={() => setActiveNav(item)}><span className="nav-icon">{navIcons[index]}</span>{item}{item === 'Alertas' && alertCount > 0 && <em>{alertCount}</em>}</button>)}</nav><div className="sidebar-bottom"><div className="security-card"><span className="security-dot" /> API protegida<strong>{keys.length ? 'Conectada' : 'Esperando configuración'}</strong></div><button className="profile" onClick={logout}><span className="avatar">FU</span><span><strong>Administrador</strong><small>Cerrar sesión</small></span><span className="chevron">↗</span></button></div></aside><main className="main-area"><header className="topbar"><div><div className="breadcrumbs">ForgeUnits Control Panel <span>/</span> {activeNav}</div><h1>{activeNav}</h1></div><div className="top-actions"><span className="live-status"><i />{loading ? 'Sincronizando' : 'Servicio activo'}</span><button className="icon-button" onClick={toggleTheme} title="Cambiar tema">{theme === 'dark' ? '☼' : '☾'}</button></div></header><div className="content">{error && <div className="error-banner">{error}<button onClick={refresh}>Reintentar</button></div>}{activeNav === 'Resumen' && renderOverview()}{activeNav === 'API keys' && renderKeys()}{activeNav === 'Consumo' && renderConsumption()}{activeNav === 'Proyectos' && renderProjects()}{activeNav === 'Alertas' && renderAlerts()}</div></main></div>
}
