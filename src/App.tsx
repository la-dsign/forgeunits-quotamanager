...
function App() {
    ...
    useEffect(() => {
        if (auth === 'required' && window.location.pathname.startsWith('/dashboard')) window.location.replace('/')
    }, [auth])
    ...
    const logout = async () => { setShowAccountPanel(false); setShowNotifications(false); await fetch('/api/auth/logout', { method: 'POST', credentials: 'same-origin' }); setAuth('required') }
    const toggleFullscreen = async () => {
        try {
            if (document.fullscreenElement) await document.exitFullscreen()
            else await document.documentElement.requestFullscreen()
            setFullscreen(Boolean(document.fullscreenElement))
        } catch { setError('El navegador no permite pantalla completa en este contexto.') }
    }
    const notificationItems = [
        ...keys.filter(key => !key.configured).map(key => ({ title: key.name, text: 'Falta configurar su secreto en Railway.', tone: 'warning' })),
        ...(summary.failedRequests ? [{ title: 'Solicitudes fallidas', text: `${fmt(summary.failedRequests)} eventos terminaron con error.`, tone: 'danger' }] : []),
    ]
    ...
    if (auth !== 'authenticated') return <div className="auth-transition" aria-label="Cargando Quotamanager" />
    ...
    return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark brand-logo"><img src="/forgeunits-logo-transparent.png" alt="ForgeUnits" /></span><div><strong>ForgeUnits <span>Quota Manager</span></strong><small>AI usage management</small></div></div><div className="workspace-label">WORKSPACE</div><div className="workspace"><span className="workspace-icon">F</span><span>ForgeUnits · IA</span></div><nav>{navItems.map((item, index) => <button key={item} className={`nav-item ${activeNav === item ? 'active' : ''}`} onClick={() => setActiveNav(item)}><span className="nav-icon">{navIcons[index]}</span>{item}{item === 'Alertas' && alertCount > 0 && <em>{alertCount}</em>}</button>)}</nav><div className="sidebar-bottom"><div className="security-card"><span className="security-dot" /> API protegida<strong>{keys.length ? 'Conectada' : 'Esperando configuración'}</strong></div><button className="profile" onClick={logout}><span className="avatar">FU</span><span><strong>Administrador</strong><small>Cerrar sesión</small></span><span className="chevron">↗</span></button></div></aside><main className="main-area"><header className="topbar"><div><div className="breadcrumbs">ForgeUnits Quota Manager <span>/</span> {activeNav}</div><h1>{activeNav}</h1></div><div className="top-actions"><span className="user-chip">Luis Alberto Palacio Ripoll</span><button className="header-action" onClick={() => setShowAccountPanel(true)}>Cuenta</button><button className="header-action" onClick={toggleFullscreen}>{fullscreen ? 'Reducir' : 'Expandir'}</button><button className="header-action notification-button" onClick={() => setShowNotifications(value => !value)}>Notificaciones{alertCount > 0 && <span className="notification-badge">{alertCount}</span>}</button><button className="header-action" onClick={logout}>Salir</button><span className="live-status"><i />{loading ? 'Sincronizando' : 'Conectado'}</span><button className="icon-button" onClick={toggleTheme} title="Cambiar tema">{theme === 'dark' ? '☼' : '☾'}</button></div>{showNotifications && <div className="header-popover notifications-popover"><div className="popover-title"><strong>Notificaciones importantes</strong><button onClick={() => setShowNotifications(false)}>×</button></div>{notificationItems.length ? notificationItems.map(item => <div className={`notification-item ${item.tone}`} key={item.title}><i /> <div><strong>{item.title}</strong><p>{item.text}</p></div></div>) : <p className="popover-empty">No hay alertas operativas.</p>}</div>}</header><div className="content">{error && <div className="error-banner">{error}<button onClick={refresh}>Reintentar</button></div>}{activeNav === 'Resumen' && renderOverview()}{activeNav === 'API keys' && renderKeys()}{activeNav === 'Consumo' && renderConsumption()}{activeNav === 'Proyectos' && renderProjects()}{activeNav === 'Alertas' && renderAlerts()}</div></main>{showAccountPanel && <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && setShowAccountPanel(false)}><section className="modal-card account-panel"><div className="modal-header"><div><p className="eyebrow">WORKSPACE</p><h3>Cuenta</h3></div><button className="close-button" onClick={() => setShowAccountPanel(false)}>×</button></div><div className="account-identity"><span className="avatar large">FU</span><div><strong>Luis Alberto Palacio Ripoll</strong><small>Administrador de Quotamanager</small></div></div><div className="account-detail"><span>Estado de sesión</span><strong>Activa y protegida</strong></div><div className="account-detail"><span>Secretos</span><strong>Gestionados en Railway</strong></div><div className="modal-actions"><button className="secondary-button" onClick={() => setShowAccountPanel(false)}>Cerrar</button><button className="primary-button" onClick={logout}>Cerrar sesión</button></div></section></div>}</div>
}
