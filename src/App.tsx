...
function App() {
    ...
    const [keyForm, setKeyForm] = useState({ id: '', name: '', projectId: '', internalLimit: '2000' })
    ...
    const saveKeyMetadata = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            await readJson('/api/ai-usage/keys', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...keyForm, internalLimit: Number(keyForm.internalLimit) }) })
            setShowKeyForm(false); setKeyForm({ id: '', name: '', projectId: '', internalLimit: '2000' }); await refresh()
    ...
    if (auth !== 'authenticated') return <div className="auth-shell"><main className="auth-layout"><section className="auth-brief"><div className="auth-brand"><span className="auth-brand-mark"><img src="/forgeunits-logo-transparent.png" alt="" /></span><span><strong>ForgeUnits</strong><small>Workspace access</small></span></div><div className="auth-logo"><img src="/forgeunits-logo-transparent.png" alt="ForgeUnits" /></div><p className="auth-wordmark">FORGE<span>UNITS</span></p><p className="auth-subtitle">QUOTAMANAGER · CONTROL PANEL</p><p className="auth-brief-copy">Supervisa el consumo de IA, revisa límites y mantiene cada decisión de uso trazable desde un solo centro operativo.</p><ul className="auth-signals"><li>Consumo y costos en tiempo real</li><li>Claves y proyectos protegidos</li><li>Alertas operativas verificables</li></ul></section><form className="auth-card" onSubmit={login}><div className="auth-card-head"><div><p className="eyebrow">QUOTAMANAGER</p><h1>Iniciar sesión</h1><p className="muted">Accede al panel de control de tu workspace.</p></div><span className="secure-mark">◉</span></div>{auth === 'checking' ? <p className="muted auth-loading">Comprobando acceso…</p> : <><label>Contraseña<input type="password" value={password} onChange={event => setPassword(event.target.value)} autoFocus required /></label><button className="primary-button" type="submit">Entrar al dashboard</button>{loginError && <p className="auth-error">{loginError}</p>}</>}</form></main></div>
    ...
    const renderKeys = () => <section className="panel full-panel"><div className="section-header"><div><p className="eyebrow">CREDENCIALES</p><h2>API keys</h2><p className="muted">Identificadores y métricas. Los secretos viven únicamente en Railway.</p></div><div className="section-actions"><label className="search"><span>⌕</span><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Buscar clave o proyecto…" /></label><button className="primary-button" onClick={() => setShowKeyForm(true)}>＋ Registrar alias</button></div></div>{filteredKeys.length ? <div className="table-wrap"><table><thead><tr><th>Identificador</th><th>Proyecto</th><th>Configuración</th><th>Solicitudes</th><th>Costo</th><th>Último uso</th></tr></thead><tbody>{filteredKeys.map((key, index) => <tr key={key.id}><td><div className="key-name"><span className={`key-symbol ${keyColor(index)}`}>⌁</span><div><strong>{key.name}</strong><small>{key.id}</small></div></div></td><td className="project-name">{key.projectId}</td><td><span className={`status-pill ${key.configured ? 'activo' : 'alerta'}`}><i />{key.configured ? 'Configurada' : 'Sin secreto'}</span></td><td><strong>{fmt(key.requests)}</strong><small className="cell-muted"> · {fmt(key.failedRequests)} fallidas</small></td><td>{money(key.costUsd)}</td><td className="last-used">{dateTime(key.lastUsed)}</td></tr>)}</tbody></table></div> : <EmptyState title="No hay API keys registradas" description="Registra un alias aquí y configura después su secreto en Railway. Esta pantalla nunca mostrará las API keys reales." />}{showKeyForm && <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && setShowKeyForm(false)}><form className="modal-card" onSubmit={saveKeyMetadata}><div className="modal-header"><div><p className="eyebrow">CONFIGURACIÓN SEGURA</p><h3>Registrar alias de API key</h3></div><button type="button" className="close-button" onClick={() => setShowKeyForm(false)}>×</button></div><p className="muted">Solo guardaremos metadatos para identificar la clave. El secreto se configura en Railway como <code>GEMINI_API_KEYS_JSON</code> o <code>GEMINI_API_KEY</code>.</p><label>Alias interno<input required value={keyForm.id} onChange={event => setKeyForm({ ...keyForm, id: event.target.value })} placeholder="project-01-main" /></label><label>Nombre visible<input required value={keyForm.name} onChange={event => setKeyForm({ ...keyForm, name: event.target.value })} placeholder="Proyecto principal" /></label><label>Project ID<input required value={keyForm.projectId} onChange={event => setKeyForm({ ...keyForm, projectId: event.target.value })} placeholder="gen-lang-client-0015125690" /></label><label>Límite interno RPM<input type="number" min="1" max="1000000" required value={keyForm.internalLimit} onChange={event => setKeyForm({ ...keyForm, internalLimit: event.target.value })} /></label>{keyFormError && <p className="auth-error">{keyFormError}</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setShowKeyForm(false)}>Cancelar</button><button className="primary-button" type="submit">Guardar alias</button></div></form></div>}</section>
