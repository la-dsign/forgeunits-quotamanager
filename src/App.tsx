...
function App() {
    ...
    if (auth !== 'authenticated') return <div className="auth-shell"><main className="auth-layout"><section className="auth-brief" aria-label="ForgeUnits Control Panel"><div className="auth-brand"><span className="auth-brand-mark"><img src="/forgeunits-logo-transparent.png" alt="" /></span><span><strong>ForgeUnits</strong><small>Workspace access</small></span></div><figure className="auth-logo"><img src="/forgeunits-logo-transparent.png" alt="ForgeUnits" /><figcaption>Control Panel</figcaption></figure><p className="auth-wordmark">FORGE<span>UNITS</span></p><p className="auth-subtitle">QUOTAMANAGER</p><p className="auth-brief-copy">Supervisa el consumo de IA, revisa límites y mantiene cada decisión de uso trazable desde un solo centro operativo.</p><ul className="auth-signals" aria-label="Capacidades del panel"><li>Consumo y costos en tiempo real</li><li>Claves y proyectos protegidos</li><li>Alertas operativas verificables</li></ul></section><form className="auth-card" onSubmit={login} aria-labelledby="loginTitle"><div className="auth-card-head"><div><p className="eyebrow">QUOTAMANAGER</p><h1 id="loginTitle">Iniciar sesión</h1><p className="muted">Accede al panel de control de tu workspace.</p></div><span className="secure-mark" aria-label="Acceso protegido">◉</span></div>{auth === 'checking' ? <p className="muted auth-loading">Comprobando acceso…</p> : <><label>Contraseña<input id="password" name="password" type="password" value={password} onChange={event => setPassword(event.target.value)} autoFocus required autoComplete="current-password" /></label><button className="primary-button" id="loginBtn" type="submit">Entrar al dashboard</button>{loginError && <p className="auth-error" role="alert">{loginError}</p>}</>}</form></main></div>
